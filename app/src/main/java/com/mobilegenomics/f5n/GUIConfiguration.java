...
public class GUIConfiguration {
    ...
    public static ArrayList<Step> getArticPipelineAutoGeneratedSteps(Context context, int windowSize,
        ...
        JsonArray argsJsonArray = _getArticPipelineCommands(context);
        for (JsonElement element : argsJsonArray) {
            ...
            if (pipelineStep.getValue() == PipelineStep.NANOPOLISH_VARIANT) {
                ...
                for (int i = 0; i < windowSize; i++) {
                    ...
                    command = command.replace("[WINDOW]", windowWithId);
                    command = command.replace("[VARIANT_OUTPUT]", variantVCFWithId);
    ...
    public static ArrayList<ArticPipelineArgument> getArticPipelineUserFilledArgs(Context context) {
        ...
        JsonArray argsJsonArray = _getArticPipelineUserFilledArgs(context);
        for (JsonElement element : argsJsonArray) {
    ...
    private static JsonArray _getArticPipelineUserFilledArgs(Context context) {
        JsonObject argsJson = JSONFileHelper.rawtoJsonObject(context, R.raw.artic_auto_generate_arguments);
        return argsJson.getAsJsonArray("user_filled_args");
    }

    private static JsonArray _getArticPipelineAutoFilledArgs(Context context) {
        JsonObject argsJson = JSONFileHelper.rawtoJsonObject(context, R.raw.artic_auto_generate_arguments);
        return argsJson.getAsJsonArray("auto_filled_args");
    }

    private static JsonArray _getArticPipelineCommands(Context context) {
        JsonObject argsJson = JSONFileHelper.rawtoJsonObject(context, R.raw.artic_auto_generate_arguments);
        return argsJson.getAsJsonArray("commands");
    }
